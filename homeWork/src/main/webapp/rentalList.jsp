...
<body>
	...
	<h1>Rental List</h1>
	<form action="/homeWork/rentalList.jsp" method="get">
		Store : 
		<select name ="storeId">
	...
	<table>
		...
		%>
			...
			<tr>
				...
				<th><a class="nolink" href="/homeWork/filmTitleDetail.jsp?filmTitle=<%=map.get("filmTitle")%>"><%=map.get("filmTitle") %></a></th>
	...
	<form action="/homeWork/rentalList.jsp">
		filmTitle Search word :
		<input type="text" name="searchWord" value="<%=searchWord%>">
		<input type="hidden" name="storeId" value="<%=storeId %>">  <!-- 지점 선택하고 검색을 해도 지점 유지 -->
		<button type="submit">검색</button>
		<a href= "/homeWork/rentalList.jsp?">초기화</a>
	</form>
	<!-- 페이징 -->
		<a href="/homeWork/rentalList.jsp?currentPage=1&storeId=<%=storeId %>&searchWord=<%=searchWord%>">[처음]</a>
		<%
			// 10개 이전 할 경우
			if(currentPage > 1) {
		...
		%>
			<a href="/homeWork/rentalList.jsp?currentPage=<%=prevPage %>&storeId=<%=storeId %>&searchWord=<%=searchWord%>">[이전]</a>
		<%
			}
			for(int i = startPage; i<= endPage; i++) {
		%>
			<a href="/homeWork/rentalList.jsp?currentPage=<%=i %>&storeId=<%=storeId %>&searchWord=<%=searchWord%>">[<%=i %>]</a>
		<%
			}
		%>	
		<%
		...
		%>
			<a href="/homeWork/rentalList.jsp?currentPage=<%=nextPage %>&storeId=<%=storeId %>&searchWord=<%=searchWord%>">[다음]</a>
		<%
			}
		%>
			<a href="/homeWork/rentalList.jsp?currentPage=<%=lastPage %>&storeId=<%=storeId %>&searchWord=<%=searchWord%>">[마지막]</a>
</body>
</html>

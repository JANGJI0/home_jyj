...
<body>
	...
	<h1>인벤토리 목록</h1>
		<table border="1">
			...
			%>
				<tr>
					...
					<td><a class="nolink" href="/homeWork/inventoryOne.jsp?filmId=<%=map.get("filmId") %>"><%=map.get("isRental") %></td>
				</tr>
			<%
				}
			%>
		...
		<!-- 검색  -->
		<form action="/homeWork/inventoryList.jsp">
		<select name="key">
		...
		<a href= "/homeWork/inventoryList.jsp?">초기화</a>
		</form>
		<!-- 페이징 -->
		<a href="/homeWork/inventoryList.jsp?currentPage=1&key=<%=key %>&searchWord=<%=searchWord %>">[처음]</a>
		<%
			// 10개 이전 할 경우
			if(currentPage > 1) {
		...
		%>
			<a href="/homeWork/inventoryList.jsp?currentPage=<%=prevPage %>&key=<%=key %>&searchWord=<%=searchWord %>">[이전]</a>
		<%
			}
			for(int i = startPage; i<= endPage; i++) {
		%>
			<a href="/homeWork/inventoryList.jsp?currentPage=<%=i %>&key=<%=key %>&searchWord=<%=searchWord %>">[<%=i %>]</a>
		<%
			}
			// 10개 다음 할 경우
		...
		%>
			<a href="/homeWork/inventoryList.jsp?currentPage=<%=nextPage %>&key=<%=key %>&searchWord=<%=searchWord %>">[다음]</a>
		<%
			}
		%>
			<a href="/homeWork/inventoryList.jsp?currentPage=<%=lastPage %>&key=<%=key %>&searchWord=<%=searchWord %>">[마지막]</a>
</body>
</html>
...
<body>
	...
	%>
		<tr>
			...
			<td><a class="nolink" href="/homeWork/filmOne.jsp?filmId=<%=map.get("filmId") %>"><%=map.get("title") %></td>
			<td><%=map.get("name") %></td>
	...
	<form action="/homeWork/filmList.jsp">
	...
	<!--  페이징 -->
		<a href="/homeWork/filmList.jsp?currentPage=1&category=<%=category %>&searchWord=<%=searchWord %>">[처음]</a>
	<%
		// 10개 이전 할 경우
		if(currentPage > 1) {
	...
	%>
		<a href="/homeWork/filmList.jsp?currentPage=<%=prevPage %>&category=<%=category %>&searchWord=<%=searchWord %>">[이전]</a>
	<%
		}
		for(int i = startPage; i<= endPage; i++) {
	%>
		<a href="/homeWork/filmList.jsp?currentPage=<%=i %>&category=<%=category %>&searchWord=<%=searchWord %>">[<%=i %>]</a>
	<%
		}
	%>
	<%
	...
	%>
		<a href="/homeWork/filmList.jsp?currentPage=<%=nextPage %>&category=<%=category %>&searchWord=<%=searchWord %>">[다음]</a>
	<%
		}
	%>
		<a href="/homeWork/filmList.jsp?currentPage=<%=lastPage %>&category=<%=category %>&searchWord=<%=searchWord %>">[마지막]</a>
</body>
</html>

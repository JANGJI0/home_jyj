...
<body>
	...
	%>
		<tr>
			...
			<td><a class="nolink" href="/homeWork/actorOne.jsp?actorId=<%=map.get("actorId") %>"><%=map.get("actor") %></a></td>
		</tr>
	<%
			}
	%>
	</table>
	<form action="/homeWork/actorList.jsp" method="get">
		<input type="text" name="searchWord" value="<%=searchWord %>">
		<button type="submit">검색</button>
		<a href= "/homeWork/actorList.jsp">초기화</a>
	</form>
	<!--  페이징 -->
		<a href="/homeWork/actorList.jsp?currentPage=1&searchWord=<%=searchWord %>">[처음]</a>
	<%
		// 10개 이전 할 경우
		if(currentPage > 1) {
	...
	%>
		<a href="/homeWork/actorList.jsp?currentPage=<%=prevPage %>&searchWord=<%=searchWord %>">[이전]</a>
	<%
		}
		for(int i = startPage; i<= endPage; i++) {
	%>
		<a href="/homeWork/actorList.jsp?currentPage=<%=i %>&searchWord=<%=searchWord %>">[<%=i %>]</a>
	<%
		}
	%>
	<%
	...
	%>
		<a href="/homeWork/actorList.jsp?currentPage=<%=nextPage %>&searchWord=<%=searchWord %>">[다음]</a>
	<%
		}
	%>
		<a href="/homeWork6/actorList.jsp?currentPage=<%=lastPage %>&searchWord=<%=searchWord %>">[마지막]</a>
</body>
</html>